...
export const healthCheckSchema = v.object({
	...
	frustration: v.optional(
		v.pipe(
			...
			v.union([
				v.literal(''),
				v.pipe(v.string(), v.maxLength(500, 'Please keep your response under 500 characters'))
			])
		)
	),
	numberOfComputers: v.optional(
		v.pipe(
			...
			v.union([
				v.literal(''),
				v.pipe(v.string(), v.regex(/^(1-3|4-6|7-10|11-20|20\+)$/, 'Please select a valid option'))
			])
		)
	)
});
