<script>
  ...
  $: if ($currentSection !== -1 && !visible && (cardsVisible || headerVisible || ctaVisible || bottomCtaVisible)) {
...
<section id="how-it-works" class="relative h-full flex items-center justify-center py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
  <!-- Background gradient -->
  <div class="absolute inset-0 bg-gradient-to-b from-[#12121a] to-[#1a1a2e]"></div>
  
  <!-- Floating glows -->
  <div class="absolute top-[30%] left-[5%] w-[400px] h-[400px] rounded-full bg-gradient-radial from-[#4dd36f]/20 to-transparent blur-[100px] opacity-50"></div>
  <div class="absolute bottom-[20%] right-[10%] w-[500px] h-[500px] rounded-full bg-gradient-radial from-[#4dd3ff]/20 to-transparent blur-[100px] opacity-50"></div>
  
  <div class="relative z-10 max-w-7xl mx-auto transition-all duration-500" style="opacity: {visible ? 1 : 0}; visibility: {visible ? 'visible' : 'hidden'};">
    <!-- Section title -->
    <div class="text-center mb-12 min-h-[120px]">
      ...
      <p 
        ...
        style="opacity: {headerVisible ? 1 : 0}; transform: translateY({headerVisible ? 0 : 30}px); transition-delay: 100ms;"
      >
    ...
    <div class="text-center mb-16 min-h-[200px]">
      ...
      <div 
        ...
        style="opacity: {ctaVisible ? 1 : 0}; transform: translateY({ctaVisible ? 0 : 30}px); transition-delay: 150ms;"
      >
    ...
    <div class="process-cards-grid grid md:grid-cols-3 gap-8 min-h-[300px]">
      {#each processSteps as step, i}
        ...
        >
          ...
          <div class="absolute -inset-[2px] bg-gradient-to-br {step.color} rounded-2xl opacity-60 animate-[gradient-rotate_4s_ease_infinite]" style="animation-delay: {i * 1.3}s"></div>
          
          <!-- Card content -->
          <div class="relative bg-[#0b090a] rounded-2xl p-10 text-center h-full">
            <!-- Number -->
            <div class="font-bold mb-6 gradient-text-custom" style="font-size: 6rem; background: linear-gradient(135deg, var(--color-from), var(--color-to)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; --color-from: {step.color.includes('4dd36f') ? '#4dd36f' : step.color.includes('4dd3ff') ? '#4dd3ff' : '#ff4d8f'}; --color-to: {step.color.includes('3fb55f') ? '#3fb55f' : step.color.includes('4158D0') ? '#4158D0' : '#C850C0'};">
              {step.number}
            </div>
    ...
    <div class="text-center mt-16 min-h-[150px]">
      <p 
        ...
        style="opacity: {bottomCtaVisible ? 1 : 0}; transform: translateY({bottomCtaVisible ? 0 : 30}px);"
      >
      ...
      <a 
        ...
        style="opacity: {bottomCtaVisible ? 1 : 0}; transform: translateY({bottomCtaVisible ? 0 : 30}px); transition-delay: 150ms;"
      >
...
<style>
  ...
  @keyframes number-glow {
    ...
    100% {
      filter: drop-shadow(0 0 30px currentColor) drop-shadow(0 0 60px currentColor) drop-shadow(0 0 80px currentColor);
    }
  }
