...
<section id="what-you-get" class="relative h-full flex items-center justify-center py-20 px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid lg:grid-cols-2 gap-12 items-start">
        ...
        <div>
          <h2 
            ...
            style="opacity: {textVisible ? 1 : 0}; transform: translate3d(0, 0, 0) scale({textVisible ? 1 : 0.95}); will-change: transform, opacity;"
          >
            What You Get
          </h2>
          ...
          <div class="space-y-4" style="min-height: {textVisible ? 'auto' : '300px'};">
            {#each features as feature, i}
              <div 
                ...
                style="opacity: {textVisible ? 1 : 0}; transform: translate3d(0, 0, 0) scale({textVisible ? 1 : 0.95}); transition-delay: {200 + (i * 100)}ms; will-change: transform, opacity;"
              >
        ...
        <div class="relative">
          ...
          <div class="absolute bottom-0 right-0 w-[400px] h-[400px] bg-gradient-radial from-[#4dd3ff]/20 via-[#4dd36f]/15 to-transparent rounded-full blur-3xl opacity-60 pointer-events-none -z-10" style="transform: translate(40px, 80px);"></div>
          ...
          <div class="space-y-6">
            ...
            <div class="feature-cards-grid grid grid-cols-1 sm:grid-cols-3 gap-5">
              {#each serviceCards as card, i}
                <div 
                  class="feature-card relative group {$currentSection === -1 ? '' : 'transition-all'}"
                  style="opacity: {$currentSection === -1 ? 1 : (cardsVisible ? 1 : 0)}; transform: {$currentSection === -1 ? 'none' : 'translate3d(0, 0, 0) scale(' + (cardsVisible ? 1 : 0.9) + ')'}; transition-duration: {$currentSection === -1 ? '0ms' : '600ms'}; transition-delay: {cardsVisible ? i * 100 : 0}ms; transition-property: opacity, transform; will-change: {$currentSection === -1 ? 'auto' : 'transform, opacity'};"
                >
                  <!-- Animated gradient border -->
                  <div class="absolute -inset-1 bg-gradient-to-br {card.gradient} rounded-2xl blur opacity-75 group-hover:opacity-100 animate-[gradient-rotate_4s_ease_infinite] transition-all duration-300"></div>
                  
                  <!-- Brighter hover glow -->
                  <div class="absolute -inset-2 bg-gradient-to-br {card.gradient} rounded-2xl blur-xl opacity-0 group-hover:opacity-80 transition-all duration-300"></div>
                  
                  <!-- Card content -->
                  <div class="relative bg-[#0b090a] rounded-2xl p-6 text-center">
                    <div class="w-12 h-12 mb-4 text-white mx-auto flex items-center justify-center" style="min-width: 48px; min-height: 48px; background: rgba(255, 255, 255, 0.1); border-radius: 8px;">
                      {#if i === 0}
                        <!-- Shield Check Icon -->
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="white" style="width: 48px; height: 48px;">
          ...
          >
            ...
            <div class="absolute bg-gradient-to-br from-[#4158D0] via-[#C850C0] to-[#FFCC70] rounded-2xl blur-xl opacity-70 animate-[pulse-glow_3s_ease-in-out_infinite] -z-10" style="left: 49%; top: 48%; right: -56%; bottom: -56%;"></div>
            <div class="absolute -inset-[2px] bg-gradient-to-br from-[#4158D0] via-[#C850C0] to-[#FFCC70] rounded-2xl opacity-90 animate-[gradient-rotate_3s_ease_infinite]"></div>
            ...
            <div class="relative bg-[#0b090a] rounded-2xl p-8 text-center">
              ...
              {#if $scheduleCallEnabled}
                <a href="/contact#schedule" class="text-[#4dd3ff] hover:text-[#4dd36f] transition-colors text-sm font-semibold">
