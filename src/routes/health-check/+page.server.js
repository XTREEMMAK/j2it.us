  ...
  default: async ({ request, getClientAddress }) => {
    ...
    try {
      ...
      const sanitizedData = {
        ...
        phone: validatedData.phone ? DOMPurify.sanitize(validatedData.phone, { ALLOWED_TAGS: [] }) : '',
        frustration: validatedData.frustration ? DOMPurify.sanitize(validatedData.frustration, { ALLOWED_TAGS: [] }) : '',
        formType: 'health-check'
      };
      
      // Send to webhook
      const webhookResult = await sendToWebhook(sanitizedData, clientIp, N8N_HEALTH_CHECK_WEBHOOK_URL);
